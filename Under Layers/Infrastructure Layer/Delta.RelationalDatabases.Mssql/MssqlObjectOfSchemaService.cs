...
{
    public class MssqlObjectOfSchemaService 
    {

        readonly DatabaseOperatedBySQL Database;
        MssqlObjectOfSchemaService(DatabaseOperatedBySQL aDatabase)
        {
            Database = aDatabase;
        }

        public static MssqlObjectOfSchemaService Of(DatabaseOperatedBySQL aDatabase)
        {
            return new MssqlObjectOfSchemaService(aDatabase);
        }
        ...
        {
            //WITH (FORMAT csv,DELIMITER ',', HEADER, ENCODING 'UTF8',FORCE_NOT_NULL ({force_not_null_columns}))");
            Database.ExecuteNonQuery($"BULK INSERT {aTable.FullName} FROM '{csv_file_path}' WITH (FIRSTROW = 2, FORMAT = 'CSV')");//FIELDTERMINATOR = ',',
        }
        //public void CopyByCsv(string csv_file_path, Table aTable, string force_not_null_columns)
        //{
        ...
        public void Create(Table table, string ddlScript, bool isRecreate)
        {
            ...
            //executeCommandTexts.Add($"ALTER USER WITH DEFAULT_SCHEMA = {table.SchemaName}");
            if (isRecreate) executeCommandTexts.Add($"DROP TABLE IF EXISTS {table.SchemaName}.{table.Name}");//TODO CASCADE");
        ...
        {
            return $"DROP TABLE {aTable.FullName} CASCADE";
        }
        void CreateTableLike(Table aSourceTable, Table aTargetTable)
        {
            Database.ExecuteNonQuery($"SELECT * INTO {aTargetTable.FullName} FROM {aSourceTable.FullName} ");
        }
        ...
        public void Truncate(Table aTable)
        {
            ...
            //Database.ExecuteNonQuery($"DBCC CHECKIDENT ([{aTable.FullName}],, RESEED, 0)");
        }
        ...
        public int InsertValues(Table table, IEnumerable<IEnumerable<string>> valuesList)
        {
            ...
            var insertedCount = 0;
            valuesList.Chunk(1000).ToList().ForEach(valuesList1000 =>
            {
                var valuesStrings1000 = valuesList1000.Select(values => $"({string.Join(",", values)})").Aggregate((all, cur) => $"{all},{cur}");
                var query = $"INSERT INTO {table.FullName} ({joinedColumnNames}) VALUES {valuesStrings1000};";
                insertedCount += Database.ExecuteNonQuery(query);
            });
